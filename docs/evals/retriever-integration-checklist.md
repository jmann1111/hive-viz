# Retriever Integration Checklist

This checklist ties together the direct chat endpoint, retrieval engine, retriever UI, and provider comparison loop.

## Architecture Contract

- [ ] Direct retrieval chat goes to a dedicated retriever endpoint, not OpenClaw
...
- [ ] The endpoint contract normalizes cleanly to `retriever-response-v1`
...
- [ ] Response payload stays short enough for the retriever to feel minimal

## Frontend Retriever

- [ ] `resolve` opens the validated file directly
- [ ] `candidates` renders a 3 to 5 option chooser with no hidden extra candidates
- [ ] `clarify` asks exactly one narrow question in the retriever UI
- [ ] The retriever never executes UI navigation on an unvalidated path
- [ ] The retriever keeps the response visually minimal and retrieval-first
- [ ] Walt interpretation, synthesis, or advising is a separate path from retrieval

## Provider Comparison

- [ ] All providers are tested against the same `docs/evals/retriever-benchmark-pack.json`
...
Current request body at `/api/retriever/retrieve`:
...
2. Start the retriever API server if you are evaluating live providers.
